...
def eval_cli_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Based on otherseq files, count lineages.',
        prog='write_otherseq_report.py',
        add_help=False
    )
...
def write_count_report(arguments):
    ...
    df1 = pd.read_csv(arguments.otherseq1, sep='\t')
    df1['end'] = 'R1'
    df2 = pd.read_csv(arguments.otherseq2, sep='\t')
    df2['end'] = 'R2'
    ...
    if arguments.mode == 'nuc':
        fh_out.write('=' * 75 + '\n')
        fh_out.write('NUCLEOTIDE OTHER SEQUENCES HIT COUNTS\n')
        fh_out.write('=' * 75 + '\n\n')
    elif arguments.mode == 'transnuc':
        fh_out.write('=' * 75 + '\n')
        fh_out.write('TRANSLATED NUCLEOTIDE OTHER SEQUENCES HIT COUNTS\n')
        fh_out.write('=' * 75 + '\n\n')
...
if __name__ == '__main__':
    ...
    # We will calculate, and report on, other sequences lineage counts.
    # Blank input will produce blank output files.

    arguments = eval_cli_arguments()

    if os.stat(arguments.otherseq1).st_size == 0 and os.stat(arguments.otherseq2).st_size == 0:
